...
def authentcate():
    ...
    def login():
        ...
        if username == "1" and password == "1":
            root1.destroy()
            subprocess.call(["python", "admin_control_panel.py"])
        else:
            try:
                ...
                # Execute an SQL query to check if the entered faculty_id and password are correct
                ...
                # If the result is not None, the entered username and password are correct
                if result is not None:
                    root1.destroy()
                    os.system(f"python faculty.py {username}")
                else:
                    # Display an error message if the entered username and password are incorrect
                    error_label.config(text="Invalid username or password")

            except mysql.connector.Error as error:
                error_label.config(text="Error while connecting to Database")
            finally:
                mycursor.close()
                mydb.close()
    ...
    header_label = tk.Label(
        root1, text="Welcome", font=("Helvetica", 24, "bold"), bg="#3253ad"
    )
    header_label.pack(pady=10)

    # Create the username label and entry field
    username_label = tk.Label(
        root1, text="Username:", font=("Helvetica", 12, "bold"), bg="#3253ad"
    )
    username_label.pack()
    ...
    password_label = tk.Label(
        root1, text="Password:", font=("Helvetica", 12, "bold"), bg="#3253ad"
    )
    password_label.pack()
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # title of software
        bg_img = ctk.CTkLabel(
            self, image=self.photoimg4, width=1530, height=850, text=""
        )
        ...
        frame = ctk.CTkFrame(
            master=self, width=1920, height=100, corner_radius=0, fg_color="#274187"
        )
        frame.place(x=0, y=0)
        label = ctk.CTkLabel(
            frame,
            text="Facial Recognition Based Attendance System               ",
            font=("Arial", 50, "bold"),
            fg_color="#274187",
            text_color="white",
        )
        ...
        frame3 = ctk.CTkFrame(
            master=self, width=1920, height=61, corner_radius=0, fg_color="#3253ad"
        )
        ...
        login_as_admin_button = ctk.CTkButton(
            frame3,
            text="Login as Admin",
            command=authentcate,
            width=50,
        ...
        login_as_faculty_button = ctk.CTkButton(
            frame3,
        ...
        curr_day = ctk.CTkLabel(
            left_frame, text=f"{date:%A}", font=("helvetica", 18), text_color="black"
        )
        ...
        curr_time = ctk.CTkLabel(left_frame, text_color="black", font=("helvetica", 18))
        ...
        curr_sub = ctk.CTkLabel(
            left_frame, text=curr_subject, font=("helvetica", 18), text_color="black"
        )
        curr_sub.grid(row=4, column=1, pady=5)

        # Semester
        sem_label = ctk.CTkLabel(
            left_frame,
            text="Semester\t\t:",
        ...
        curr_sem = ctk.CTkLabel(
            left_frame, text="6th", font=("helvetica", 18), text_color="black"
        )
        curr_sem.grid(row=5, column=1, pady=5)

        # Attend
        # curr_date = dt.datetime.today().strftime('%Y-%m-%d')
        # sql = "SELECT sum FROM "+str(curr_sub)+" WHERE date = %s"
        # cursor.execute(sql, (curr_date,))
        # row=cursor.fetchone()
        # attend=row[0]
        attend = 64

        attend_label = ctk.CTkLabel(
            left_frame,
            text="Attend\t\t:",
            font=("helvetica", 18, "bold"),
            text_color="black",
        ...
        curr_text = str(attend) + "/124"
        curr_attend = ctk.CTkLabel(
            left_frame, text=curr_text, font=("helvetica", 18), text_color="black"
        )
        ...
        team_frame = ctk.CTkFrame(
            self, width=600, height=500, corner_radius=0, fg_color="white"
        )
        team_frame.grid(row=0, column=1, pady=(120, 0))

        # logo
        img1 = Image.open(r"Image/team.png").resize((850, 600), Image.LANCZOS)
        self.photoimg1 = ImageTk.PhotoImage(img1)

        t_lbl = ctk.CTkLabel(team_frame, image=self.photoimg1, text="")
        t_lbl.grid(row=0, column=0, columnspan=2)

        # detect face button
        img6 = Image.open("Image/facialRec.png").resize((200, 200), Image.LANCZOS)
        ...
        b1_1.place(x=500, y=610)

        frame1 = ctk.CTkFrame(master=self, width=1920, height=60, corner_radius=0)
        frame1.place(x=0, y=800)
        label1 = ctk.CTkLabel(
            frame1,
            text="Group 9 Project Made under Supervison of Dr. Mantosh Bishwas                                  ",
